...
def collect-oa3-urls [spec_data: record] {
  mut urls = []
  # root-level servers
  let root_servers = ($spec_data.servers? | default [])
...
def get-body-info-impl [op: record, schemas: record] {
  ...
  if ($request_body | is-empty) {
    ...
    if ($found_ct != null) {
      ...
      let first_ct = ($content | columns | first | default $spec.CT_JSON)
      {has_body: true, body_schema: {}, content_type: $first_ct}
    }
  }
}

def get-response-content-types-impl [op: record] {
  ...
  for code in $spec.RESPONSE_CODE_PRIORITY {
    if ($found_schema == null) {
      ...
      if ($resp | is-not-empty) {
        ...
        let json_schema = ($content | get -o "application/json" | default {} | get -o schema)
