...
def base-url-completer [] { ["https://custom.example.com" "https://dumbo.k8s.elmec.ad"] }
...
export def "cloud-init create" [
  ...
  network_type: string@network-type-completer # * `dhcp` - dhcp * `static` - static
  otp_vpn: string
  --ip-address: string
  --gateway: string
  --primary-dns: string
  ...
  let req_body = {"networkType": $network_type, "otpVpn": $otp_vpn, "ipAddress": $ip_address, "gateway": $gateway, "primaryDns": $primary_dns, "secondaryDns": $secondary_dns} | compact
...
export def "dmilog list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"customer": $customer, "domain": $domain, "name": $name, "ordering": $ordering, "private_ip": $private_ip, "search": $search, "vpn_ip": $vpn_ip} | compact), body: null}
}
...
export def "hooks-chart create" [
  ...
  let req_body = {"version": $version, "project_id": $project_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "hooks-dmilog create" [
  ...
  let req_body = {"sha": $sha, "project_id": $project_id, "commit_message": $commit_message} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "services list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ordering": $ordering, "search": $search} | compact), body: null}
}
