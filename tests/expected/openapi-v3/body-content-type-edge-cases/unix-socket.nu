# Auto-generated client for Body Content-Type Edge Cases v1.0.0
# Source: <spec>
# Auth: --token flag or $env.BODY_CONTENT_TYPE_EDGE_CASES_TOKEN
...
def pet-type-completer [] { ["cat" "dog"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "json create" } } | get name | first)
...
export def "json create" [
  ...
  let req_body = {"name": $name, "count": $count, "active": $active, "nickname": $nickname} | compact
...
export def "upload-empty upload" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
# POST /oneof
#
# Discriminator (request): petType = cat, dog
# operationId: createOneof
export def "oneof create" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --unix-socket(-U): string@unix-socket-completer # Connect to the API over a Unix socket instead of TCP
  pet_type: string@pet-type-completer
  --meow: string
  --bark: string
]: any -> any {
  let input = $in
