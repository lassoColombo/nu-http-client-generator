# Auto-generated client for path-and-enum-edge-cases v1.0.0
# Source: <spec>
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PATH_AND_ENUM_EDGE_CASES_TOKEN | default "" }
...
def base-url-completer [] { ["https://custom.example.com" "https://example.com"] }
...
export def "sites list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"anchorsite_override": $anchorsite_override} | compact), body: null}
}
...
export def "sites-active list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "anchorsite_override" $anchorsite_override "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/sites/active" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"anchorsite_override": $anchorsite_override} | compact), body: null}
}
...
# GET /quoted-enum
#
# operationId: quoted_enum_get
export def "quoted-enum get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --literal-quoted: string@literal-quoted-completer
]: nothing -> record {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "literal_quoted" $literal_quoted "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/quoted-enum" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"literal_quoted": $literal_quoted} | compact), body: null}
}
