...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PATH_AND_ENUM_EDGE_CASES_TOKEN | default "" }
...
export def "sites-active list" [
  ...
  let sock = ($unix_socket | default $UNIX_SOCKET)
  let qp = [(serialize-qp "anchorsite_override" $anchorsite_override "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/sites/active" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "quoted-enum get" [
  ...
  let full_url = (build-url $base "/quoted-enum" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"literal_quoted": $literal_quoted} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
