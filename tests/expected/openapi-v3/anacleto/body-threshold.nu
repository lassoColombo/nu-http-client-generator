...
export def "carpet-asset-list-idrs list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ordering": $ordering, "page": $page, "per_page": $per_page, "search": $search} | compact), body: null}
}
...
export def "delivery-asset list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"address": $address, "idrs": $idrs, "is_active": $is_active, "is_public": $is_public, "is_scannable": $is_scannable, "name": $name, "ordering": $ordering, "page": $page, "per_page": $per_page, "scanner_name": $scanner_name, "search": $search, "tag": $tag, "type": $type} | compact), body: null}
}
...
export def "delivery-asset-create create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: record # shape: {idrs: string, name: string, is_scannable: bool, scanner_id: int, type: "HOSTNAME"|"CUSTOM", address: list, tag_id?: int}
...
export def "delivery-asset-details update" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: record # shape: {name: string, is_scannable: bool, atlantis_id?: int, is_active?: bool, type: "HOSTNAME"|"CUSTOM"|"CI"|"NETWORK", address: list, scanner_id: int, error?: string, tag_id?: int}
...
export def "delivery-asset-whitelist create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: record # shape: {ips: list, creation_comment: "Under decommissioning"|"Risk transferred"|"Risk accepted", expiration_date?: string, description?: string}
