...
# --body shape: {id?: int, name: string, category?: record, photoUrls: list<string>, tags?: list, status?: "available"|"pending"|"sold"}
export def "pet update" [
  ...
  --body: record # shape: {id?: int, name: string, category?: record, photoUrls: list<string>, tags?: list, status?: "available"|"pending"|"sold"}
...
export def "pet update-with-form" [
  ...
  let full_url = (build-url $base ({pet_id: (encode-path-segment $pet_id)} | format pattern "/pet/{pet_id}") $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"name": $name, "status": $status} | compact), body: null}
}
...
export def "pet-upload-image upload-file" [
  ...
  let full_url = (build-url $base ({pet_id: (encode-path-segment $pet_id)} | format pattern "/pet/{pet_id}/uploadImage") $qp)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/octet-stream" $req_body {query: ({"additionalMetadata": $additional_metadata} | compact), body: $req_body}
}
...
export def "store-order create-place" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: record # shape: {id?: int, petId?: int, quantity?: int, shipDate?: string, status?: "placed"|"approved"|"delivered", complete?: bool}
...
export def "user create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
  --body: record # shape: {id?: int, username?: string, firstName?: string, lastName?: string, email?: string, password?: string, phone?: string, userStatus?: int}
