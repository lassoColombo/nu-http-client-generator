# Auto-generated client for Scalar Response Edge Cases v1.0.0
# Source: <spec>
# Auth: --token flag or $env.SCALAR_RESPONSE_EDGE_CASES_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SCALAR_RESPONSE_EDGE_CASES_TOKEN | default "" }
...
export def "count get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let sock = ($unix_socket | default $UNIX_SOCKET)
  let full_url = (build-url $base "/count" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "healthy get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let sock = ($unix_socket | default $UNIX_SOCKET)
  let full_url = (build-url $base "/healthy" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "etag get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let sock = ($unix_socket | default $UNIX_SOCKET)
  let full_url = (build-url $base "/etag" $auth.query)
