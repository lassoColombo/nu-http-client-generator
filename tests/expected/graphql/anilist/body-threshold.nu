...
# --anime-list-options shape: {sectionOrder?: string, splitCompletedSectionByFormat?: bool, customLists?: string, advancedScoring?: string, advancedScoringEnabled?: bool, theme?: string}
# --manga-list-options shape: {sectionOrder?: string, splitCompletedSectionByFormat?: bool, customLists?: string, advancedScoring?: string, advancedScoringEnabled?: bool, theme?: string}
...
# --started-at shape: {year?: int, month?: int, day?: int}
# --completed-at shape: {year?: int, month?: int, day?: int}
...
export def "mutation update-media-list-entries" [
  ...
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"status": $status, "score": $score, "scoreRaw": $score_raw, "progress": $progress, "progressVolumes": $progress_volumes, "repeat": $repeat, "priority": $priority, "private": $private, "notes": $notes, "hiddenFromStatusLists": $hidden_from_status_lists, "advancedScores": $advanced_scores, "startedAt": $started_at, "completedAt": $completed_at, "ids": $ids} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id userId mediaId status score progress progressVolumes repeat priority private notes hiddenFromStatusLists customLists advancedScores updatedAt createdAt" }
    let body = {query: ("mutation($status: MediaListStatus, $score: Float, $scoreRaw: Int, $progress: Int, $progressVolumes: Int, $repeat: Int, $priority: Int, $private: Boolean, $notes: String, $hiddenFromStatusLists: Boolean, $advancedScores: [Float], $startedAt: FuzzyDateInput, $completedAt: FuzzyDateInput, $ids: [Int]) { UpdateMediaListEntries(status: $status, score: $score, scoreRaw: $scoreRaw, progress: $progress, progressVolumes: $progressVolumes, repeat: $repeat, priority: $priority, private: $private, notes: $notes, hiddenFromStatusLists: $hiddenFromStatusLists, advancedScores: $advancedScores, startedAt: $startedAt, completedAt: $completedAt, ids: $ids) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
  }
