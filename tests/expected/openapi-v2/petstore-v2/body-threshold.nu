...
export def "pet-upload-image upload-file" [
  ...
  let full_url = (build-url $base ({pet_id: $pet_id} | format pattern "/pet/{pet_id}/uploadImage"))
  let req_body = {"additionalMetadata": $additional_metadata, "file": $file} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req_body = if ($file | is-not-empty) { $req_body | upsert file (open -r $file) } else { $req_body }
...
# POST /pet
# operationId: addPet
# --body shape: {id?: int, category?: record, name: string, photoUrls: list<string>, tags?: list, status?: "available"|"pending"|"sold"}
export def "pet create" [
  ...
  --body: record # shape: {id?: int, category?: record, name: string, photoUrls: list<string>, tags?: list, status?: "available"|"pending"|"sold"}
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
