...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "pet-upload-image uploadFile" } } | get name | first)
...
export def "pet addPet" [
  ...
  --body: record # shape: {id?: int, category?: record, name: string, photoUrls: list, tags?: list, status?: "available"|"pending"|"sold"}
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
