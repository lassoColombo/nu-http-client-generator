...
def base-url-completer [] { ["https://custom.example.com" "https://petstore.swagger.io/v2" "http://petstore.swagger.io/v2"] }
...
export def "pet addPet" [
  ...
  let body = {"id": $id, "category": $category, "name": $name, "photoUrls": $photo_urls, "tags": $tags, "status": $status} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
