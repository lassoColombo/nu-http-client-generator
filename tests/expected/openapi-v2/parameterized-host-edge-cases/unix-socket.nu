# Auto-generated client for Parameterized Host Edge Cases v2 v1.0.0
# Source: <spec>
# Auth: --token flag or $env.PARAMETERIZED_HOST_EDGE_CASES_V2_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PARAMETERIZED_HOST_EDGE_CASES_V2_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "indexes list" } } | get name | first)
